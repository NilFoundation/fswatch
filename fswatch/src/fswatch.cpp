...
struct printf_event_callbacks event_format_callbacks {
    print_event_flags, print_event_path, print_event_timestamp
};
...
static void print_event_timestamp(const event &evt) {
    ...
    std::string date = strftime(time_format_buffer, TIME_FORMAT_BUFF_SIZE, tformat.c_str(), tm_time) ?
                           std::string(time_format_buffer) :
...
static void start_monitor(int argc, char **argv, int optind) {
    ...
    if (mflag)
        active_monitor = monitor_factory::create_monitor(monitor_name, paths, process_events);
    else
        active_monitor =
            monitor_factory::create_monitor(fsw_monitor_type::system_default_monitor_type, paths, process_events);
    ...
    for (const auto &filter_file : filter_files) {
        auto filters_from_file = monitor_filter::read_from_file(
            filter_file, [](std::string f) { std::cerr << _("Invalid filter: ") << f << "\n"; });

        std::move(filters_from_file.begin(), filters_from_file.end(), std::back_inserter(filters));
    }
...
static void parse_opts(int argc, char **argv) {
    ...
    while ((ch = getopt_long(argc, argv, short_options.c_str(), long_options, &option_index)) != -1) {
#else
    ...
    if (format_flag && (tflag || xflag)) {
        std::cerr << _("--format is incompatible with any other format option such as -t and -x.") << std::endl;
...
static int printf_event_validate_format(const std::string &fmt) {
    ...
    struct printf_event_callbacks noop_callbacks {
        format_noop, format_noop, format_noop
    };
