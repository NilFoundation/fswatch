...
namespace fsw {
    ...
    void kqueue_monitor::initialize_kqueue() {
        if (kq != -1)
            throw libfsw_exception(_("kqueue already running."));
    ...
    int kqueue_monitor::wait_for_events(const std::vector<struct kevent> &changes,
        ...
        int event_num = kevent(kq, &changes[0], (int)changes.size(), &event_list[0], (int)event_list.size(), &ts);
    ...
    void kqueue_monitor::process_events(const std::vector<struct kevent> &changes,
        ...
        for (auto i = 0; i < event_num; ++i) {
            ...
            if (e.fflags) {
                events.push_back({load->file_names_by_descriptor[e.ident], curr_time, decode_flags(e.fflags)});
            }
        }
