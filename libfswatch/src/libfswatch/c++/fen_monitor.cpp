...
namespace fsw {
    ...
    struct fen_monitor_load {
        ...
        struct fen_info *get_descriptor_by_name(const string &path) {
            ...
            if (res == descriptors_by_file_name.end())
                return nullptr;
    ...
    fen_monitor::fen_monitor(vector<string> paths_to_monitor, FSW_EVENT_CALLBACK *callback, void *context) :
    ...
    bool fen_monitor::add_watch(const string &path, const struct stat &fd_stat) {
        ...
        if (watch_access)
            finfo->events |= FILE_ACCESS;
        if (!follow_symlinks)
            finfo->events |= FILE_NOFOLLOW;

        // if the descriptor could be opened, track it
        if (associate_port(finfo, fd_stat)) {
    ...
    bool fen_monitor::scan(const string &path, bool is_root_path) {
        ...
        if (!is_dir && !is_root_path && directory_only)
            return true;
        if (!is_dir && !accept_path(path))
            return true;
        if (!is_dir)
    ...
    void fen_monitor::process_events(struct fen_info *finfo, int event_flags) {
        ...
        if (event_flags & FILE_DELETE)
            load->descriptors_to_remove.insert(finfo);
        else
            load->paths_to_rescan.insert(finfo->fobj.fo_name);
    ...
    void fen_monitor::rescan_removed() {
        ...
        while (fd != load->descriptors_to_remove.end()) {
            ...
            if (!port_dissociate(load->port, PORT_SOURCE_FILE, reinterpret_cast<uintptr_t>(&finfo->fobj)) != 0) {
