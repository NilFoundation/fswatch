...
#define FSW_MONITOR_NOTIFY_GUARD
    ...
    monitor::monitor(std::vector<std::string> paths, FSW_EVENT_CALLBACK *callback, void *context) :
    ...
    void monitor::add_filter(const monitor_filter &filter) {
        ...
        try {
            ...
            throw libfsw_exception(string_utils::string_from_format(_("An error occurred during the compilation of %s"),
                                                                    filter.text.c_str()),
