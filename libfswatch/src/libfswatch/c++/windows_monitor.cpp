...
namespace fsw {
    ...
    windows_monitor::windows_monitor(vector<string> paths_to_monitor, FSW_EVENT_CALLBACK *callback, void *context) :
    ...
    void windows_monitor::initialize_events() {
        for (const wstring &path : load->win_paths) {
            ...
            if (h == NULL)
                throw libfsw_exception(_("CreateEvent failed."));
    ...
    bool windows_monitor::init_search_for_path(const wstring path) {
        ...
        HANDLE h = CreateFileW(path.c_str(), GENERIC_READ, FILE_SHARE_DELETE | FILE_SHARE_READ | FILE_SHARE_WRITE,
    ...
    void windows_monitor::process_path(const wstring &path) {
        ...
        if (!is_path_watched(path)) {
            if (!init_search_for_path(path))
                return;
        }

        auto it = load->dce_by_path.find(path);
        if (it == load->dce_by_path.end())
            throw libfsw_exception(_("Initialization failed."));
