...
namespace fsw {
    ...
    bool inotify_monitor::add_watch(const std::string &path, const struct stat &fd_stat) {
        ...
        int inotify_desc = inotify_add_watch(impl->inotify_monitor_handle, path.c_str(), IN_ALL_EVENTS);
    ...
    void inotify_monitor::scan(const std::string &path, const bool accept_non_dirs) {
         ...
         */
        if (!is_dir && !accept_non_dirs)
            return;
        if (!is_dir && directory_only)
            return;
        if (!accept_path(path))
            return;
    ...
    void inotify_monitor::process_pending_events() {
        ...
        std::for_each(impl->paths_to_rescan.begin(), impl->paths_to_rescan.end(), [this](const std::string &p) {
            this->scan(p);
        });
