...
namespace fsw {
    ...
    fsevents_monitor::fsevents_monitor(vector<string> paths_to_monitor, FSW_EVENT_CALLBACK *callback, void *context) :
    ...
    void fsevents_monitor::run() {
        ...
        for (const string &path : paths) {
            dirs.push_back(CFStringCreateWithCString(nullptr, path.c_str(), kCFStringEncodingUTF8));
        }
        ...
        CFArrayRef pathsToWatch =
            CFArrayCreate(nullptr, reinterpret_cast<const void **>(&dirs[0]), dirs.size(), &kCFTypeArrayCallBacks);
    ...
    void fsevents_monitor::on_stop() {
        if (!run_loop)
            throw libfsw_exception(_("run loop is null"));
